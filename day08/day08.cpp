...
auto part_two() -> const int {
  ...
  constexpr std::bitset<7> one{0B0010010};
  constexpr std::bitset<7> four{0B0111010};
  const std::unordered_map<std::bitset<7>, int> seg_dict = {
      {0B1110111, 0}, {one, 1},       {0B1011101, 2}, {0B1011011, 3},
      {four, 4},      {0B1101011, 5}, {0B1101111, 6}, {0B1010010, 7},
      {0B1111111, 8}, {0B1111011, 9}};
  constexpr std::bitset<7> a{0B1000000};
  constexpr std::bitset<7> b{0B0100000};
  constexpr std::bitset<7> c{0B0010000};
  constexpr std::bitset<7> d{0B0001000};
  constexpr std::bitset<7> e{0B0000100};
  constexpr std::bitset<7> f{0B0000010};
  constexpr std::bitset<7> g{0B0000001};
  std::ifstream fin("../input.txt");
  std::string s;
  int sum = 0;
  while (getline(fin, s, '|')) {
    std::unordered_map<std::bitset<7>, std::string> word_cipher;
    ...
    for (const auto& [ch, i] : symbol_count) {
      switch (i) {
        ...
        case 8:  // both 'c' & 'a' occur 8 times, use 'one' to decide
          if (word_cipher[one].find(ch) != std::string::npos) {
            char_cipher[ch] = c;
          } else {
            char_cipher[ch] = a;
          }
        ...
        case 7:  // both 'd' & 'g' occur 7 times, use 'four' to decide
          if (word_cipher.at(four).find(ch) != std::string::npos) {
            char_cipher[ch] = d;
          } else {
            char_cipher[ch] = g;
          }
    ...
    for (int i = 3; i >= 0; --i) {
      ...
      std::bitset<7> b;
      for (const char& c : s) {
        b |= char_cipher[c];  // OR char bits to find displayed number
      }
